...
//Variável que controla o estilo de câmera: look-at ou free-cam
bool g_LookAt = false;
...
bool pause = false;
bool flagRing[QUANT_RINGS] = {true, true, true, true, true, true, true, true, true, true};
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(1000, 600, "INF01047 - Richard", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    glm::vec4 ringPosition [QUANT_RINGS];
    ringPosition[0] = {0.0f, 5.0f, 5.0f, 1.0f};
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glm::vec4 camera_position_c_teste;
        ...
        if (g_FreeCam){
            camera_position_c_teste = camera_position_c_g;
            camera_lookat_l_teste = camera_lookat_l_g;
            camera_up_vector_teste = camera_up_vector_g;
        }
        ...
        * Matrix_Rotate_Z(g_AirplaneAngle);
        ...
        for(int i = 0; i < QUANT_RINGS; i++)
        {
            if(flagRing[i])
            {
                ...
                p1 = p1 + ringPosition[i];
                p2 = p2 + ringPosition[i];
                p3 = p3 + ringPosition[i];
                p4 = p4 + ringPosition[i];
                p5 = p5 + ringPosition[i];
                p6 = p6 + ringPosition[i];
                p7 = p7 + ringPosition[i];
...
void CursorPosCallback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    if (!g_LeftMouseButtonPressed)
        return;
    if (g_LeftMouseButtonPressed && g_LookAt) {
        ...
        camera_lookat_l_g =  glm::vec4(x,y,z,1.0f); // Ponto "l", para onde a câmera (look-at) estará sempre olhando
    }
